...
Results for all rapidity bins are shown in ???.
...
done with \textsc{NLOJET++} program v4.1.2 (?citace?) using CT10 NLO PDF set
(?citace?). 
...
of jets observed in each $\pt$ bin. Comparison for other rapidity bins is shown
???.
...
\begin{figure}[t]
  ...
  \caption{Comparison of NLO QCD prediction of double differential inclusive jet
    ...
    remaining rapidity bins are shown in ???.}
...
uncertainties are taken into account. These include (?citace?)
...
\begin{itemize}
  ...
  \item \textbf{$\alpha_S$ uncertainty}
    ...
    Figure \ref{fig:RunningCouplingConstant}.
...
\begin{figure}[p]
  ...
  \caption{Comparison of \textsc{Pythia8} prediction with NLO QCD prediction of
    ...
    Comparisons for other rapidity bins are shown in ???.}
